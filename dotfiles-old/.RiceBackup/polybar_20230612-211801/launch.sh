...
launch_bar() {
    ...
    polybar -q main -c "$dir/$style/main_config.ini" &
    polybar -q main -c "$dir/space/second_config.ini" &
  elif [[ "$style" == "default" ]]; then
    polybar -q example -c "$dir/$style/main_config.ini" &
    polybar -q example -c "$dir/$style/second_config.ini" &
	else
    exec "$dir/$style/launch.sh" &
	fi
}
...
if [[ "$1" == "--material" ]]; then
  ...
  launch_bar
	...
	--shades    --shapes         --space     --space-doom
  --osx       --powerline      --default
	EOF
fi
